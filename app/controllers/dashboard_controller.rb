...
#//////////////Admin:
  ...
  def get_users_for_company
    # Ignore auto-created admin user:
    users = User.where("email not LIKE '%@admin.admin'").where(:company_id => current_user.company_id)
    @admins = users.where(:is_an_admin => true, :is_deleted => false, :company_id => current_user.id, :is_super_admin => nil).order('is_active desc, first_name, last_name') # Removed :is_active => true for just admin users' view
    @coaches = User.where(:is_an_admin => false, :is_a_coach => true, :is_deleted => false, :is_super_admin => nil).order('is_active desc, first_name, last_name') # Removed :is_active => true for just admin users' view
    @users = User.where(:is_an_admin => false, :is_a_coach => false, :is_deleted => false, :is_super_admin => nil).order('is_active desc, first_name, last_name') # Removed :is_active => true for just admin users' view
    @users_to_confirm = User.where(:is_a_coach => nil, :company_id => current_user.id, :is_super_admin => nil).order('first_name, last_name')
    @deleted_users = User.where(:is_deleted => true, :is_super_admin => nil).order('first_name, last_name')
  end
